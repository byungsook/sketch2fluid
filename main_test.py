...
def main():
    ...
    if DEVICE.type == 'cpu':
        pix2vox_pair.load_state_dict(
            torch.load(opt.resume + '/pix2vox_pair_iter-2000.pth', map_location={'cuda:0': 'cpu'}))
    else:
    ...
    for f, s in zip(front_list, side_list):
        s_front = _preprocess_single_sketch(f, sigma=opt.sigma).to(DEVICE)
        s_side = _preprocess_single_sketch(s, sigma=opt.sigma).to(DEVICE)
        ...
        utils.save_render(renderer, gen_den_rot,
                          opt.data + '\\' + save_path[0] + '\\' + f.split('\\')[-1].split('.')[0] + '.png')
        utils.save_render(renderer, patch_sketcher.rotate(gen_den_rot, 'xn'),
                          opt.data + '\\' + save_path[1] + '\\' + s.split('\\')[-1].split('.')[0] + '.png')
        np.savez_compressed(opt.data + '\\' + save_path[2] + '\\' + f.split('\\')[-1].split('.')[0] + '.npz',
                            density=gen_den_rot[0, 0].detach().cpu().numpy())
        np.savez_compressed(opt.data + '\\' + save_path[3] + '\\' + s.split('\\')[-1].split('.')[0] + '.npz',
                            density=patch_sketcher.rotate(gen_den_rot, 'xn')[0, 0].detach().cpu().numpy())

    t2 = time()
    print('Elapsed time (include saving time): {:.4f}'.format(t2 - t1))
